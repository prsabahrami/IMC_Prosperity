...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        coef = [4.999041e+03,  5.830593e-02, -4.869977e-04,  5.829545e-07,  5.793575e-09, -3.037098e-11,  6.805529e-14, -8.510963e-17,  6.322860e-20, -2.772803e-23, 6.634538e-27, -6.683339e-31]
        ...
        for product in state.order_depths:
            ...
            if len(self.prices[product]) == 10 and product == 'STARFRUIT':
                ...
                acc_prices[product] = coef[0] + coef[1] * t + coef[2] * t ** 2 + coef[3] * t ** 3 + coef[4] * t ** 4 + coef[5] * t ** 5 + coef[6] * t ** 6 + coef[7] * t ** 7 + coef[8] * t ** 8 + coef[9] * t ** 9 + coef[10] * t ** 10 + coef[11] * t ** 11
            ...
            if len(order_depth.sell_orders) != 0:
                best_ask, best_ask_amount = list(sorted(order_depth.sell_orders.items(), reverse=True))[0]
                if int(best_ask) < acceptable_price:
                    print("BUY", str(-best_ask_amount) + "x", best_ask)
                    orders.append(Order(product, best_ask, -best_ask_amount))
    
            if len(order_depth.buy_orders) != 0:
                best_bid, best_bid_amount = list(sorted(order_depth.buy_orders.items()))[0]
